...
## Load Impact Area areas file
```{r}
impact_file <- list.files(input_dir, pattern = "Impact_areas", full.names = TRUE)
```
...
# Build Impact Area FVS inputs
```{r}
stand_table <- tm_stand_init(area_file = impact_file, 
              blank_stand_init = blank_stand, 
              proj_name = proj_name, 
              tm_year = tm_year) 
tree_table <- tm_tree_init(area_file = impact_file,
           blank_tree_init = blank_tree,
           tm_tree_list = tm_trees)
basename <- tools::file_path_sans_ext(basename(impact_file))
basename <- gsub("_areas", "", basename)
file <- paste0(output_dir, "/FVS_input_", basename, ".xlsx")
write_xlsx(list(FVS_StandInit=stand_table, FVS_TreeInit=tree_table), file)
print(paste("Created FVS inputs saved as", file))

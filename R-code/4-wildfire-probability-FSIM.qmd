...
### Final treatment shapefiles 

Note: If shapefiles were separated or combined in Script 1, the below shapefiles reflect those changes.
```{r}
shp_files <- list.files(
  paste(final_polygons_dir, project_ID, sep = "/"),
  pattern = "\\.shp$",
  recursive = TRUE,
  full.names = TRUE
)
...
for (shp_path in shp_files) {
  ...
  mean_results <- rbind(
    ...
    data.frame(
      ...
      mean_value = paste0(round(m_val*100, 3), "%")
    )
  )
  plot(clipped_raster)
  plot(v_shp, add = T)
...
results <- mean_results %>% 
  rename("Annual probability of wildfire occurrence" = mean_value)
results <- flextable(results)
results <- bold(results, j = 2)
results <- fontsize(results, size = 14, part = "all")
results <- width(results, width = 4)
results
```

**Tip**: When you enter this value into the Calculator tool, double check that the decimal point is in the right place!
...
```{r}
df <- read_excel(paste0(fvs_output_dir, "/8GG24602_3.1_PM.xlsx"))
```

## Load the data tables with areas
```{r}
areas <- read_excel(paste0(areas_dir, "/8GG24602_3.1_areas.xlsx"))
```
...
# Calculate the biomass of carbon removed via mechanical treatments

### Sum LIVECREM and Convert to BDT
```{r}
if("TRNF" %in% df_list){
  df_trnf <- df %>% 
    filter(MgmtID=="TRNF")
  LIVECREM <- sum(df_trnf$LIVECREM)
  removed_BDT <- LIVECREM/0.50/0.907185
  print(paste("Biomass removed via mechanical treatments (BDT) =", removed_BDT))
}
```

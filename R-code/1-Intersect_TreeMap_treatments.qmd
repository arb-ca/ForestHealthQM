...
```{r}
trt_dir <- "~/CCI/QMs/Forest Health QM/Test runs/new 2025 projects/8GG24601/Shapefiles"
```
...
for(i in 1:nrow(buff_combined)){
  ...
  shapefile_files <- list.files(
    path = triaads_output_dir,
    pattern = paste0("^", shp_name, "\\.(shp|shx|dbf|prj)$"),
    full.names = TRUE
  )
  
  zipfile_path <- file.path(triaads_output_dir, paste0(shp_name, ".zip"))
  zip(zipfile_path, files = shapefile_files)
  # Delete the original shapefile components
...
for (i in 1:length(shp_list)) {
  ...
  shapefile_files <- list.files(
    path = triaads_output_dir,
    pattern = paste0("^", shapefile_name, "\\.(shp|shx|dbf|prj)$"),
    full.names = TRUE
  )
  
  zipfile_path <- file.path(triaads_output_dir, paste0(shapefile_name, ".zip"))
  zip(zipfile_path, files = shapefile_files)

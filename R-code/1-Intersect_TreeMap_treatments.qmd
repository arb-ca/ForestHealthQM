...
areas_table <- full_join(treemap_areas_df, shp_areas_df) %>% 
...
## Plot all treatments and highlight overlaps
```{r}
...
print("Matrix of overlapping TCNs:")
print(overlap_list)
...
## Plot only treatments that overlap
```{r}
# Find overlapping areas
overlaps <- st_intersection(combined) %>%
  filter(n.overlaps > 1) %>%  # Keep only actual overlaps
...
ggplot() +
  geom_sf(data = combined_overlap, aes(fill = as.factor(TCN), col = as.factor(TCN)), alpha = 0.5) +
  geom_sf(data = overlaps, fill = "black", color = "black", alpha = 0.6) +
  theme_minimal() +
  labs(title = "Only overlapping TCNs -- Overlaps are Black",
       fill = "TCN", 
       col = "TCN")
```

# Plot each treatment group (e.g. 1.1 and 1.2) together
```{r}

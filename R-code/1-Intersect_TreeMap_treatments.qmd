...
for(i in 1:length(shp_list)){
  ...
  if(nrow(shp)>1){
    shp_list <- shp_list[-i]
    shp_list <- c(shp_list, split(shp, seq_len(nrow(shp))))
  } else{
    shp_list_new <- c(shp_list, shp)
  }
}
...
# NEW STEP: Combine shapefiles according to `combine` var
```{r}
if(length(combine)>0){
...
## Do the same for TCN
```{r, warning = F}
TCN_list <- sapply(TCN_list, unique)
...
areas_table <- areas_table %>% 
  mutate(index = seq(1:nrow(areas_table))) %>% 
  mutate(Shapefile =  tools::file_path_sans_ext(basename(shapefiles[index])))

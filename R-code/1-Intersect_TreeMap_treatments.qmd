...
# Add shapefile as column in shp
```{r}
...
for(i in 1:length(shp_list)){
  ...
  if(nrow(shp)>1){
    ...
    print(paste("SPLITTING TCN =", shp$TCN))
...
for (i in 1:length(shp_list)) {
  ...
  print(paste("Created TreeMap Polygons for trt area of TCN", unique(shp_sf$TCN)))
...
#### Identify PM treatments that need impact area calculations
```{r}
...
for(i in 1:nrow(buffers_final)){
  ...
  print(paste("Created TreeMap Polygons for impact area of TCN", shp_buff_sf$TCN))

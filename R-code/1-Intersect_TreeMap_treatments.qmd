...
#### For each other polygon, add it to `ignore` if it overlaps >50% with `keep`
```{r, warning=FALSE}
ignore <- c()
...
#### Remove `ignore` from `buff_combined`
```{r}
buff_combined <- buff_combined %>% 
  filter(!TCN %in% ignore)
```

#### Repeat the process with the next biggest treatment as `keep2`
```{r}
keep2 <- buff_combined %>% 
  filter(!TCN == keep$TCN) %>% 
  mutate(trt_area = as.numeric(trt_area)) %>% 
...
for(i in 1:nrow(buff_combined)){
  buff <- buff_combined[i,]
  if(buff$TCN == keep2$TCN){
    next
  }
  inter <- st_intersection(buff, keep2)
  ggplot(inter)+
    geom_sf(fill = "pink")+
    geom_sf(data = buff, fill = "green", alpha = 0.3)
  if (nrow(inter) > 0) {

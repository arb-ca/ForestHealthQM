...
fvs_varloc_sf <- st_as_sf(fvs_varloc)
```
...
aoi_prep <- function(aoi) {
  ...
  aoi <- st_zm(aoi)
  aoi <- st_transform(aoi, st_crs(fvs_varloc_sf))
  aoi <- vect(aoi)
  aoi <- makeValid(aoi)

  
  
  aoi_varlocs$TCN <- aoi$TCN
  aoi_varlocs$Tobj <- aoi$Tobj
  ...
  aoi_proj <- st_as_sf(aoi_varlocs) #convert to sf object
...
### Apply `aoi_prep()` to each shapefile
```{r}
shp_list_new <- lapply(shp_list, aoi_prep) #takes a second
```

### Combine all shapefiles 
```{r}
combined <- lapply(seq_along(shp_list_new), function(i) {
  map <- shp_list_new[[i]]
...
shp_areas_list <- list()
TCN_list <- list()
for (i in 1:length(shp_list_new)) {
  print(i)
  shp_sf <- shp_list_new[[i]]
  shp <- vect(shp_sf)

...
for (i in 1:length(shp_list)) {
  ...
  if(shp$TCN %in% noFVS){
    print(paste("Skipping TCN =", shp$TCN))
    next
  }
    
  # Start with impact area
  shp_buff <- st_buffer(shp, dist = buffer_dist)
  
  aoi_prepped_buff <- aoi_prep(aoi = shp_buff, 
           fvs_varloc = fvs_varloc,
           tm_rast = tmrast)

  # Clip, mask, and convert to polygons
  tm_poly_buff <- crop(tmrast, aoi_prepped_buff) %>% 
    mask(aoi_prepped_buff) %>% 
    as.polygons %>% 
    makeValid()
  # Intersect
  intersected_buff <- terra::intersect(tm_poly_buff, aoi_prepped_buff)

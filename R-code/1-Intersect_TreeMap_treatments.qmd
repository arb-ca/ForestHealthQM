...
```{r}
proj_name <- "custom polygon from Kevin, 10/7"
```

```{r}
project_ID <- "Burtoncreekflight_poly_v2"
```
...
```{r}
trt_dir <- "~/CCI/QMs/Forest Health QM/Test runs/Burtoncreekflight_poly_v2"
```
...
```{r}
# Define possible names for the TCN column
Tobj_variants <- c("Tobj", "TreatmentO", "TObj", "TObjective")

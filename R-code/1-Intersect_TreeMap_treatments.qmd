...
```{r}
trt_dir <- "~/../OneDrive - California Air Resources Board/documents/CCI/QMs/Forest Health QM/QM Runs 2025/CFIP/8GG25300 Sanctuary Forest CFIP/shapefile"
```
...
```{r}
ca_dir <- "~/../OneDrive - California Air Resources Board/documents/Reference data/CA boundary/ca-state-boundary"
```
...
triaads_output_dir <- "~/../Data_heavy_projects/Forest_QM_automation/triaads_input"
...
for(i in 1:nrow(buffers_final)){
  ...
  file_name <- paste(output_dir, "/", "IA_areas_", project_ID, "_", shp_buff_sf$TCN, ".xlsx", sep = "")
  ...
  st_write(intersected_sf_buff, triaads_output_dir, shp_name, driver = "ESRI Shapefile", delete_layer = T, quiet = T)
...
for (i in 1:length(shp_list)) {
  ...
  st_write(intersected_sf, triaads_output_dir, shapefile_name, driver = "ESRI Shapefile", delete_layer = T, quiet = T)
  ...
  file_name <- paste(output_dir, "/", project_ID, "_", shp_sf$TCN, "_areas.xlsx", sep = "")

...
```{r}
combined_shp <- lapply(seq_along(shp_list), function(i) {
  map <- shp_list[[i]]
...
ggplot(combined_shp)+
  ...
  geom_sf(data = combined_shp %>% filter(TCN==1.4), fill = "red", alpha = 0.7)+
  geom_sf(data = combined_shp %>% filter(TCN==1.6), fill = "yellow", alpha = 0.7)
...
### Create polygons around each treatment and crop them based on overlap
```{r}
buffers <- list()
for (i in 1:length(shp_list)) {
  shp_sf <- shp_list[[i]]
...
### Plot them all together, limiting to Fuel Reduction
```{r}
buff_combined <- buffers |> bind_rows() %>% 
  filter(Tobj == "Fuels Reduction") %>% 
...
### For each buffer area, if > half or more overlaps with other buffer areas, add it to a list called "Ignore"

#### Assign `keep` to the largest treatment
```{r}
keep <- buff_combined %>% 
...
#### For each other polygon, add it to `ignore` if it overlaps >80% with `keep`
```{r}
ignore <- c()
...
#### Remove those from buff_combined
```{r}
buff_combined <- buff_combined %>% 
  filter(!TCN %in% ignore)
```

#### Repeat the process with the next biggest treatment as `keep`
```{r}
keep <- buff_combined %>% 
  filter(!TCN == keep$TCN) %>% 
  mutate(trt_area = as.numeric(trt_area)) %>% 
  filter(trt_area == max(trt_area)) %>% 
  filter(!Tactivity %in% c("Pruning", "Chipping")) %>% 
  slice(1)
```
...
```{r}
grid.arrange(impact_map_full,impact_map_filtered, ncol = 2)
```


## Intersect the buffer polygons with TreeMap
```{r}
for(i in 1:nrow(buff_combined)){
  shp_buff_sf <- buff_combined[i,]
  shp_buff_sf <- st_transform(shp_buff_sf, crs(tmrast))
  shp_buff <- shp_buff_sf %>% as.polygons()
  tm_poly_buff <- crop(tmrast, shp_buff_sf, mask = T) 

  # Intersect
  intersected_sf_buff <- terra::intersect(tm_poly_buff, shp_buff_sf) %>% 
    st_as_sf()
    # Remove columns
  ...
  file_name <- paste(output_dir, "/", "IA_areas_", buff_combined$TCN, ".xlsx", sep = "")
...
for (i in 1:length(shp_list)) {
  ...
  map <- ggplot() +
    #geom_sf(data = intersected_sf_buff, aes(fill = as.factor(TM_ID)), color = NA, alpha = .2) + # This slows down the script. Comment out to speed things up. 
    ...
    geom_sf(data = shp_buff_sf, fill = NA)
...
```{r}
centroids <- st_centroid(combined %>% st_zm())

...
```{r}
proj_name <- "Butte Creek - Buck Mountain Forest Health Project"
```
...
impact_area <- st_buffer(merged_sf, dist = buffer_dist)
...
# Save shapefiles for use in impact area app
...
trt_sf <- st_union(merged_sf)
trt_string <- paste(project_ID, "trt", sep = "_")
st_write(trt_sf, output_dir_randig, trt_string, driver = "ESRI Shapefile", delete_layer = T, quiet = T)
```

### Zip 
```{r}
# Find all files with "_trt" in the name
files_to_zip <- list.files(path = output_dir_randig, trt_string, full.names = TRUE)
# Create the zip 
zip(zipfile = paste0(output_dir_randig, "/", trt_string, ".zip"), files = files_to_zip)
```

## Impact area
```{r}
ia_string <- paste(project_ID, "ia", sep = "_")
```

### Write
```{r}
st_write(impact_area, output_dir_randig, ia_string, driver = "ESRI Shapefile", delete_layer = T, quiet = T)
```

### Zip
```{r}
# Find all files with "_ia" in the name
files_to_zip <- list.files(path = output_dir_randig, ia_string, full.names = TRUE)
# Create the zip 
zip(zipfile = paste0(output_dir_randig, "/", ia_string, ".zip"), files = files_to_zip)
```

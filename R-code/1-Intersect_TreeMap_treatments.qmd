...
```{r}
trt_dir <- "~/../OneDrive - California Air Resources Board/documents/CCI/QMs/Forest Health QM/QM Runs 2025/CFIP/8GG25303 Bomont/shapefile/"
```
...
## Only for Lost Coast Redwood: merge the three shapefiles into one
```{r}
geom_list <- lapply(shp_list, st_geometry)
all_geoms <- do.call(c, geom_list)
merged_geom <- st_union(all_geoms)
merged_geom <- st_cast(merged_geom, "POLYGON")
geometry <- st_union(merged_geom)
merged_sf <- st_sf(geometry)

shp_list <- list(merged_sf)
st_write(merged_sf, dsn = "~/CCI/QMs/Forest Health QM/QM Runs 2025/WCB/Lost Coast Redwoods/Shapefiles", layer = "merged", driver = "ESRI Shapefile", delete_layer = T)
```

...
## NEW STEP: If one shapefile contains disparate polygons, separate the polygons into separate shapefiles
```{r}
shp_list_new <- list()
for(i in 1:length(shp_list)){
...
## NEW STEP: Combine shapefiles according to `combine_tcns` and `combine_tcns_2` vars
```{r}
...
```{r}
plot_all <- ggplot(combined_shp)+
  geom_sf(aes(fill = TCN), alpha = .5) +

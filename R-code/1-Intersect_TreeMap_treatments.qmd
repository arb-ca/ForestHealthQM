...
# List shapefiles in dir,  ignoring those with "buffer" in the name
```{r}
...
# Find TreeMap acres for each polygon

This is the acreage of the polygon that excludes non-forested pixels

```{r}
treemap_areas_list <- tibble(
  Name = names(treemap_areas_list),
  Value = unlist(treemap_areas_list)
) 

treemap_areas_list <- treemap_areas_list %>% 
  mutate(Value = round(Value)) %>% 
  rename("TreeMap Acres" = Value) %>% 
  rename(Shapefile = Name)

# Print a pretty table
flextable(treemap_areas_list)
```
